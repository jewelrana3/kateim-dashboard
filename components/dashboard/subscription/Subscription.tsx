...
export default function Subscription() {
  return (
    <div className="h-screen grid grid-cols-1 lg:grid-cols-4 gap-10 mt-10 px-6">
      {pricingPlans.map((plan, index) => (
        <section key={index}>
          {/* header */}
          <h1 className="bg-[#B0CBF4] py-2 text-center font-bold rounded-md">
            {plan.name}
          </h1>
          <div
            key={index}
            className="relative overflow-hidden rounded-2xl bg-white mt-4 "
          >
